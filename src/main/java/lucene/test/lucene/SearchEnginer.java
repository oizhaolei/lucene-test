...
public final class SearchEnginer {
    ...
    private SearchEnginer(String indexDir) {
        try {
            Directory directory = FSDirectory.open(new File("SearchEnginer", indexDir).toPath());
            ...
            manager = new SearcherManager(writer, (SearcherFactory)null);

            nmrt = new ControlledRealTimeReopenThread<IndexSearcher>(writer, manager, 5.0, 0.05);
    ...
    Directory initDirectory(File path) throws IOException {
        ...
        Directory dir = FSDirectory.open(path.toPath());// 装载磁盘索引
        /* RAMDirectory来访问索引其速度和效率都是非常优异的 */
        MMapDirectory map = new MMapDirectory(path.toPath());
    ...
    public void insert(Document doc) throws LuceneException {
        try {
            reopenToken = writer.addDocument(doc);
        } catch (IOException e) {
    ...
    public void update(Term term, Document doc) throws LuceneException {
        try {
            reopenToken = writer.updateDocument(term, doc);
        } catch (IOException e) {
    ...
    public void delete(Term term) throws LuceneException {
        try {
            reopenToken = writer.deleteDocuments(term);
        } catch (IOException e) {
            logger.error("Error in lucene re-indexing operation: {}", e);
    ...
    public List<JMap> like(int docNum, String[] fields) throws IOException {
        MoreLikeThis mlt = new MoreLikeThis(DirectoryReader.open(writer));
    ...
    public void searchAfterHighlight(QueryBuilder builder, PageModel<JMap> model, Set<String> fields) {
        ...
        try {
            ...
            model.setTotalCount(docs.totalHits.value);
    ...
    private String getBestFragment(Query query, String target, String fieldName) throws Exception {
        ...
        result = isBlank(result) ? target.substring(0, Math.min(200, target.length())) : result;
        return result;
    }
